...
export default {
    data() {
        ...
        let arr1 = [0, 0],
            arr2 = [0, 0];
        [0, 0].map((val, index) => {
            arr1[index] = $u.timeFormat($u.random(threeDayAgo, nowTime), 'yyyy/mm/dd hh:MM:ss');
            arr2[index] = $u.timeFormat($u.random(threeDayAgo, nowTime), 'yyyy/mm/dd hh:MM:ss');
        });
        return {
            timeArr1: arr1,
            timeArr2: arr2,
    ...
    methods: {
        timeArr1Change(index) {
            this.result = $u.timeFrom(new Date(this.timeArr1[index]).getTime());
        },
        timeArr2Change(index) {
            this.result = $u.timeFrom(new Date(this.timeArr2[index]).getTime());
        }
    }
};
</script>
